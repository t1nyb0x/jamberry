...
| トリガー | ユーザーが `/jam track <url>` コマンドを実行 |

### 基本フロー

```
1. ユーザーが `/jam track <url>` コマンドを実行する
...
| トリガー | ユーザーが `/jam recommend <url>` コマンドを実行    |

### 基本フロー

```
1. ユーザーが `/jam recommend <url>` コマンドを実行する
2. jamberry が入力をバリデーションする
3. jamberry が Discord に「処理中」表示を返す（DeferReply）
4. jamberry が入力を Spotify URL に正規化する
5. jamberry が tracktaste API にリクエストを送信する
...
| トリガー | ユーザーが `/jam artist <url>` コマンドを実行 |

### 基本フロー

```
1. ユーザーが `/jam artist <url>` コマンドを実行する
2. jamberry が入力をバリデーションする
3. jamberry が Discord に「処理中」表示を返す（DeferReply）
4. jamberry が入力を Spotify URL に正規化する
5. jamberry が tracktaste API にリクエストを送信する
...
| トリガー | ユーザーが `/jam album <url>` コマンドを実行 |

### 基本フロー

```
1. ユーザーが `/jam album <url>` コマンドを実行する
2. jamberry が入力をバリデーションする
3. jamberry が Discord に「処理中」表示を返す（DeferReply）
4. jamberry が入力を Spotify URL に正規化する
5. jamberry が tracktaste API にリクエストを送信する
...
| トリガー | ユーザーが `/jam search <query>` コマンドを実行 |

### 基本フロー

```
1. ユーザーが `/jam search <query>` コマンドを実行する
...
| -------- | ------------------------------------------------------------ |
| アクター | ユーザー（コマンド実行者）                                   |
| 事前条件 | `/jam recommend` または `/jam search` の結果が表示されている |
| 事後条件 | 次/前の 5 件が表示される                                     |
...
| -------- | ----------------------------------------------------------------------- |
| アクター | ユーザー（コマンド実行者以外）                                          |
| 事前条件 | 他ユーザーの `/jam recommend` または `/jam search` 結果が表示されている |
