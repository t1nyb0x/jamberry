...
| スコア           | 最終スコア（final_score: similarity × genre_bonus） |
| ジャンルボーナス | ジャンル一致時の倍率（1.0 以外の場合のみ表示）      |
| マッチ理由       | 類似と判定された理由（BPM 類似、タグ一致等）        |
| Spotify リンク   | 各トラックへの直接リンク                            |

> **スコア計算**: final_score = similarity_score × genre_bonus
>
> - similarity_score: BPM/Duration/Gain/タグの類似度
> - genre_bonus: ジャンル一致時のボーナス倍率（1.5: 完全一致, 1.3: 同グループ, 1.0: 関連, 0.5: 無関係）
...
- TrackTaste v2 API からは最大 50 件が返却可能（デフォルト 20 件）
- 初回表示: 5 件
- ページング: 「◀ 前へ」「次へ ▶」ボタンで 5 件ずつページ送り
- ソート: 最終スコア（final_score）降順（TrackTaste 側でソート済み）
...
├─────────────────────────────────────────────────┤
│ 1. トラック名A                                   │
│    🎤 アーティストA | スコア: 1.38 (×1.5)        │  ← final_score, genre_bonus
│    📀 アルバム名                                 │
│    🔗 Spotify                                   │
│ 2. トラック名B                                   │
│    🎤 アーティストB | スコア: 1.25 (×1.3)        │
│    📀 アルバム名                                 │
│    🔗 Spotify                                   │
│ 3. トラック名C                                   │  ← Description 内にリスト
│    🎤 アーティストC | スコア: 0.92               │  ← genre_bonus=1.0 の場合は省略
│    📀 アルバム名                                 │
│    🔗 Spotify                                   │
│ 4. トラック名D                                   │
│    🎤 アーティストD | スコア: 0.85               │
...
> **注**: スコアとジャンルボーナスは TrackTaste v2 API から提供される場合のみ表示。
> ジャンルボーナスが 1.0 の場合は省略される。
...
### 6. TrackTaste ステータス確認

TrackTaste API のヘルスチェックを行い、ステータス情報を表示します。
...
| フィールド | 説明                                                  |
| ---------- | ----------------------------------------------------- |
| Status     | API の状態（healthy / unhealthy）                     |
| Version    | TrackTaste のバージョン                               |
| Uptime     | 起動時間                                              |
| Git Commit | ビルド時の Git コミットハッシュ（存在する場合）       |
| Build Time | ビルド時刻（存在する場合）                            |
| Services   | 接続サービスの状態（✅ enabled / ❌ disabled で表示） |
...
│ ✅ redis                                        │
├─────────────────────────────────────────────────┤
│ jamberry vX.X.X                    │ 2025-12-02 │  ← Footer
└─────────────────────────────────────────────────┘
```
...
| フィールド          | 説明                                                 |
| ------------------- | ---------------------------------------------------- |
| 🎵 /jam track       | トラック情報取得の説明                               |
| 👤 /jam artist      | アーティスト情報取得の説明                           |
| 💿 /jam album       | アルバム情報取得の説明                               |
| ✨ /jam recommend   | レコメンド機能の説明（モード・スコア・ボーナス含む） |
| 🔍 /jam search      | 検索機能の説明                                       |
| 🩺 /tracktaste      | TrackTaste ステータス確認の説明                      |
| ❓ /help            | ヘルプ表示の説明                                     |
| 📝 対応する入力形式 | Spotify URL / URI / ID の説明                        |
...
| 項目     | 内容                                                |
| -------- | --------------------------------------------------- |
| トリガー | Bot へのメンション（@jamberry）                     |
| 応答     | ヘルプコマンドへのリンクを含むメッセージで返信      |
| 形式     | `</help:コマンドID>` 形式でクリック可能なリンク表示 |

#### 応答例

```
こんにちは！ 🍇
jamberry の使い方は </help:123456789> で確認できます。
```
